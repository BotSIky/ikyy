...
const starts = async (client = new WAConnection()) => {
  ...
  client.on("open", () => {
    success("2", "Masok Pak√, HAI OWNER IKY👋");
  });
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(client.base64EncodedAuthInfo(), null, "\t")
        ...
        ownerNumber = ["6283870640443@s.whatsapp.net",`6283870640443@s.whatsapp.net`]
        dtod = "6283870640443@s.whatsapp.net"
       otod = `6283870640443@s.whatsapp.net`
  ...
  client.on('group-participants-update', async (anu) => {
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Alo Tod *@${num.split('@')[0]}* Welcome In ${mdata.subject}\n\nJangan Lupa Tersenyum 🗿`
				...
				teks = `Babay:v @${num.split('@')[0]}\n\nNitip Nasgor Yek`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
			else if (anu.action == 'promote') {
fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${mdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;pemuda;;;\nFN:pemuda\nitem1.TEL;waid=6283870640443:6283870640443\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
